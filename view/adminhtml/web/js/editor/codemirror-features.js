    ...
    function controls(cm) {
        $(cm.getWrapperElement()).append(`
            <div class="mls-cm-controls">
                ...
                <button data-mls-cm data-action="toggleTheme" data-option="theme" data-value="default">
        ...
        cm.on('optionChange', (cm, option) => {
            $(cm.getWrapperElement()).find(`[data-option="${option}"]`)
                .attr('data-value', cm.getOption(option));
        });

        $(cm.getWrapperElement()).find('[data-mls-cm]').each(button => {
            $(button).attr('data-value', cm.getOption($(button).data('option')));
        });
